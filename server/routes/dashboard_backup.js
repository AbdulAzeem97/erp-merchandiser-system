...
router.get('/production-metrics', asyncHandler(async (req, res) => {
  ...
  const efficiencyResult = await dbAdapter.query(efficiencyQuery);
  ...
  const topCompaniesResult = await dbAdapter.query(topCompaniesQuery);
  ...
  const productPerformanceResult = await dbAdapter.query(productPerformanceQuery);

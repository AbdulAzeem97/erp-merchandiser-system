...
async function fixDatabaseSchema() {
  try {
    console.log('🔧 Starting database schema fix with correct credentials...');
    
    await client.connect();
    console.log('✅ Database connected successfully');

    // Check if process_sequences table exists
    const sequencesTableCheck = await client.query(`
      SELECT EXISTS (
    ...
    const offsetCheck = await client.query('SELECT id FROM process_sequences WHERE product_type = $1', ['Offset']);
